...
**Description:** Loads a video from a file and produces a video stream.

**Node Options** `<TODO>`

**Basic Node IO:** This node has no input streams, rather the input video file is specified by `node_options->path`. This node has two output streams, one of which is optional. The first output stream produces `ImagePacket` objects, the second *optional* output stream produces `VideoStreamInfoPacket` objects.
